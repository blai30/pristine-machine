...
- **Base UI components: hybrid API.** When wrapping a Base UI primitive, pick by shape. Containers whose content varies (Dialog, Popover, Menu, Accordion, Toolbar, Navbar, SideNav) export a **namespace object of pre-styled parts** (`export const Dialog = { Root, Trigger, Popup, ... }`); the `Popup` part bundles the Portal/Backdrop/Positioner so consumers never wire plumbing (see `Drawer`), and `Trigger`/`Close` stay raw Base UI parts (consumers pass `render={<Button />}`). Leaf components with fixed anatomy (Progress, Slider, Avatar, Separator) are **single convenience wrappers** with props. Type a part wrapper as `Omit<Base.Part.Props, 'className' | 'render'> & { className?: string }`. `Navbar`/`SideNav` are namespaces over Base UI `NavigationMenu` (links only — no triggers/popups); consumers own each link's `href` and `onClick` (the design system no longer scrolls or tracks navigation). `NavigationMenu.Link` throws without a `NavigationMenu.Root` ancestor, so a group header outside the link list (`SideNav.GroupLabel`) is a plain styled anchor, not a Base UI `Link`.
