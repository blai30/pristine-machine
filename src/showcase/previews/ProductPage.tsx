    ...
    <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="flex items-baseline gap-2">
          <span className="font-serif text-4xl text-mauve-900 dark:text-mauve-100">
            ${unitPrice}
          </span>
        ...
        <div className="flex flex-wrap items-end gap-4">
          <div className="flex flex-col gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="w-8 text-center font-mono text-sm text-mauve-900 dark:text-mauve-100">
                {quantity}
              </span>
              <IconButton
        ...
        <div className="flex flex-col gap-4">
          ...
          {tab === 'details' && (
            <p className="font-sans text-sm leading-relaxed text-mauve-600 dark:text-mauve-400">
              A friction-pivot arm holds any angle without knobs. The shade throws a clean, even
              pool of light — engineered for drafting tables and late-night detailing.
            </p>
          )}
          {tab === 'specs' && (
            <dl className="divide-y divide-mauve-200 dark:divide-mauve-700">
              {specs.map(([label, value]) => (
                <div key={label} className="flex items-center justify-between py-2">
                  <dt className="font-mono text-xs tracking-widest text-mauve-400 uppercase">
                    {label}
                  </dt>
          ...
          {tab === 'care' && (
            <p className="font-sans text-sm leading-relaxed text-mauve-600 dark:text-mauve-400">
              Wipe with a dry cloth. Avoid solvents on the brushed finish. The pivot can be
              re-tensioned with the included hex key.
            </p>
          )}
        </div>
      </div>
